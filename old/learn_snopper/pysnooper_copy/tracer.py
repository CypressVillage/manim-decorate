...
# output可能是一个路径，overwrite是设置，这个函数返回重定向stderr的接口
def get_write_function(output, overwrite):
    # 判断output是不是一个路径
    is_path = isinstance(output, (pycompat.PathLike, str))
    if overwrite and not is_path:
        # 如果设置了重定向但是output不是path
        raise Exception('`overwrite=True` can only be used when writing '
                        'content to file.')
    if output is None:
        # 要是没有重定向路径的内容，重定向到sys.stderr
        def write(s):
            stderr = sys.stderr
            try:
                stderr.write(s)
...
class Tracer:
    ...
    def __call__(self, function_or_class):
        # 当类被调用的时候，判断是fun还是cls，并进入对应的包裹函数
        # 禁用了就禁了
        if DISABLED:
    ...
    def _wrap_class(self, cls):
        ...
        for attr_name, attr in cls.__dict__.items():
            ...
            if pycompat.iscoroutinefunction(attr):
                continue
            # 要是类中的方法的话，就吧类里的方法也包裹起来，很好，很好
            # 这样一来只需要包裹类就能包裹类中的所有函数
    ...
    def _wrap_function(self, function):
        # 针对function的修饰器们
        # function.__code__是一个fun的信息，至少包含fun的名字和地址
        # 将遇到的fun添加到self.target_codes里去
        ...
        def generator_wrapper(*args, **kwargs):
            ...
            # 不懂
            # send是一个生成器的方法，gen是一个生成器
            # method是生成器的send方法，incoming是生成器的参数；incoming=None的作用是启动生成器
            method, incoming = gen.send, None
            while True:
                # 这里会调用__enter__方法，包裹生成器
                with self:
                    try:
