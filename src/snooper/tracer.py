...
class Tracer:
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        ...
        self.on_elapsed_time(start_time, elapsed_time_string)
        #                                                                     #
    ...
    def trace(self, frame, event, arg):
        ...
        if self.last_source_path != source_path:
            ...
            self.on_source_path(source_path)
        ...
        for name, value_repr in local_reprs.items():
            if name not in old_local_reprs:
                ...
                self.on_newish_variable(newish_string, name, value_repr)
            elif old_local_reprs[name] != value_repr:
                self.write('{indent}{_FOREGROUND_GREEN}{_STYLE_DIM}'
                           'Modified var:.. {_STYLE_NORMAL}{name} = '
                           '{value_repr}{_STYLE_RESET_ALL}'.format(**locals()))
                self.on_modified_variable(name, value_repr)
        ...
        if ended_by_exception:
            self.write('{_FOREGROUND_RED}{indent}Call ended by exception{_STYLE_RESET_ALL}'.
                       format(**locals()))
            self.on_call_ended_by_exception()
        else:
        ...
        if event == 'return':
            ...
            if not ended_by_exception:
                ...
                self.write('{indent}{_FOREGROUND_CYAN}{_STYLE_DIM}'
                           ...
                           '{_STYLE_RESET_ALL}'.
                           format(**locals()))
                self.on_return_value(return_value_repr)
